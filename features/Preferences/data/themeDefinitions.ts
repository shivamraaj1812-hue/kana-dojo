 ...
 * Theme Definitions
 *
 * All base theme color palettes organized by group (Premium, Base, Light, Dark, etc.).
 * Each theme only defines essential colors — derived colors (card, border, accents)
 * are generated at build time by themeColors.ts utilities.
 */
import {
  Atom,
  Sun,
...
export interface BaseThemeGroup {
  name: string;
  icon: LucideIcon;
  isLight: boolean;
  themes: BaseTheme[];
